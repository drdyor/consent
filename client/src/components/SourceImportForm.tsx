...
export function SourceImportForm({ onComplete }: { onComplete: () => void }) {
  const [form, setForm] = useState({ productName: "", manufacturer: "", category: "neuromodulator" as "neuromodulator" | "ha_filler" | "biostimulator" | "medical_device" | "other", activeIngredient: "", jurisdiction: "PL", language: "pl" as "pl" | "en", registryAuthority: "", registryIdentifier: "", documentTitle: "", documentUrl: "", documentVersion: "", documentKind: "spc" as "spc" | "ifu" | "pi" | "dfu", disclosureTitle: "", disclosureBody: "", scope: "product" as "product" | "area", area: "" });
  ...
  return <section className="clinical-panel mt-7 p-6">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      <div className="space-y-2"><Label>Product name</Label><Input value={form.productName} onChange={e => update("productName", e.target.value)} placeholder="Product used by your clinic" /></div>
      ...
      {form.scope === "area" && <div className="space-y-2"><Label>Area key</Label><Input value={form.area} onChange={e => update("area", e.target.value)} placeholder="e.g. lips" /></div>}
      <div className="space-y-2 md:col-span-2"><Label>Disclosure heading</Label><Input value={form.disclosureTitle} onChange={e => update("disclosureTitle", e.target.value)} placeholder="e.g. Contraindications" /></div>
      <div className="space-y-2 md:col-span-2"><Label>Exact reviewed disclosure excerpt</Label><Textarea value={form.disclosureBody} onChange={e => update("disclosureBody", e.target.value)} placeholder="Paste an exact excerpt from the canonical document; do not paraphrase or generate clinical content." className="min-h-28 rounded-xl" /></div>
    </div>
    <div className="mt-5 flex justify-end"><Button disabled={create.isPending || !canSubmit} onClick={() => create.mutate({ productName: form.productName, manufacturer: form.manufacturer, category: form.category, activeIngredient: form.activeIngredient || undefined, jurisdiction: form.jurisdiction, language: form.language, registryAuthority: form.registryAuthority || undefined, registryIdentifier: form.registryIdentifier || undefined, documentTitle: form.documentTitle, documentUrl: form.documentUrl, documentVersion: form.documentVersion, documentKind: form.documentKind, disclosures: [{ scope: form.scope, treatmentAreaKey: form.scope === "area" ? form.area : undefined, kind: "warning", title: form.disclosureTitle, body: form.disclosureBody, requiredAcknowledgement: true }] })} className="bg-[#24453e] text-white"><Plus className="mr-2 size-4" />Register for verification</Button></div>
  </section>;
}
