...
export default function Profile() {
  ...
  const [clinic, setClinic] = useState({ name: "", contactEmail: "", contactPhone: "", addressLine: "", jurisdiction: "PL", defaultLanguage: "pl" as "pl" | "en" });
  ...
  useEffect(() => {
    ...
    setClinic({ name: workspace.data.clinic.name, contactEmail: workspace.data.clinic.contactEmail || "", contactPhone: workspace.data.clinic.contactPhone || "", addressLine: workspace.data.clinic.addressLine || "", jurisdiction: workspace.data.clinic.jurisdiction || "PL", defaultLanguage: workspace.data.clinic.defaultLanguage || "pl" });
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <section className="clinical-panel p-6 sm:p-7">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-2 sm:col-span-2"><Label htmlFor="clinic-address">Address line</Label><Input id="clinic-address" value={clinic.addressLine} disabled={!isAdmin} onChange={e => setClinic({ ...clinic, addressLine: e.target.value })} placeholder="Street, locality, postcode" className="h-11 rounded-xl bg-white" /></div><div className="space-y-2"><Label>Jurisdiction</Label><Input value={clinic.jurisdiction} disabled={!isAdmin} onChange={e => setClinic({ ...clinic, jurisdiction: e.target.value.toUpperCase() })} placeholder="PL" className="h-11 rounded-xl bg-white" /></div><div className="space-y-2"><Label>Default consent language</Label><select value={clinic.defaultLanguage} disabled={!isAdmin} onChange={e => setClinic({ ...clinic, defaultLanguage: e.target.value as "pl" | "en" })} className="h-11 w-full rounded-xl border border-[#ded7cb] bg-white px-3 text-sm"><option value="pl">Polish</option><option value="en">English</option></select></div>
          </div>
        </section>
