...
describe("consent.sign treatment map integration", () => {
  it("locks map rows with their signed product, lot, expiry, practitioner, quantity, and note context", async () => {
    ...
    let selectCall = 0; let capturedUpdate: any; const directUpdates: any[] = [];
    ...
    state.db = {
      select: vi.fn(() => {
        ...
        if (selectCall === 3) { const q: any = { from: () => q, where: () => ({ orderBy: async () => [mapEntry] }) }; return q; }
        if (selectCall === 4) { const q: any = { from: () => q, where: () => q, limit: async () => [{ ...record, status: "signed", snapshotHash: "sealed-snapshot-hash", signedSnapshot: { sealed: true }, notaryAttemptCount: 0 }] }; return q; }
