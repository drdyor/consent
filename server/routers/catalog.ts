...
export const catalogRouter = router({
  ...
  createTemplate: protectedProcedure.input(z.object({ name: z.string().min(3).max(160), procedureKey: z.string().min(3).max(100), description: z.string().max(1000).optional(), clinicalModule: z.enum(["aesthetic", "dental", "medical"]).default("aesthetic"), jurisdiction: z.string().min(2).max(32).default("PL"), language: z.enum(["pl", "en"]).default("pl"), sections: templateSections })).mutation(async ({ ctx, input }) => {
    const workspace = await requireAdmin(ctx.user);
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const result = await db.insert(consentTemplates).values({ clinicId: workspace.clinic.id, createdByUserId: ctx.user.id, name: input.name, procedureKey: input.procedureKey, description: input.description, clinicalModule: input.clinicalModule, jurisdiction: input.jurisdiction, language: input.language, sections: input.sections, status: "active" }).$returningId();
  ...
  recordMarketJurisdictionEvidence: protectedProcedure.input(z.union([
    ...
    z.object({ catalogueProductId: z.number().int().positive(), market: z.literal("mt_malta"), maltaLocalMarketReference: z.string().min(2).max(160), maltaLocalMarketEvidenceUrl: optionalHttpsUrl, maltaProductEconomicOperatorName: z.string().min(2).max(200), maltaProductEconomicOperatorEvidenceUrl: optionalHttpsUrl }),
    ...
    } else if (input.market === "mt_malta") {
      if (catalogue.productClassification !== "medical_device") throw new Error("Malta local-market device evidence applies only to catalogue records classified as medical devices; other product classifications require a separate qualified review");
      if (!input.maltaLocalMarketEvidenceUrl || !input.maltaProductEconomicOperatorEvidenceUrl) throw new Error("Malta device evidence requires an HTTPS local-market reference URL and product economic-operator evidence URL");
      await db.update(marketCatalogueProducts).set({ maltaLocalMarketReference: input.maltaLocalMarketReference, maltaLocalMarketEvidenceUrl: input.maltaLocalMarketEvidenceUrl, maltaProductEconomicOperatorName: input.maltaProductEconomicOperatorName, maltaProductEconomicOperatorEvidenceUrl: input.maltaProductEconomicOperatorEvidenceUrl, maltaEvidenceVerifiedAt: verifiedAt }).where(eq(marketCatalogueProducts.id, catalogue.id));
    } else {
    ...
    const marketLabel = input.market === "uk_gb" ? "Great Britain MHRA/UKCA" : input.market === "mt_malta" ? "Malta Medicines Authority" : "USA FDA";
    await db.insert(auditEvents).values({ clinicId: workspace.clinic.id, actorUserId: ctx.user.id, action: `catalogue.${input.market}_evidence_verified`, entityType: "marketCatalogueProduct", entityId: String(catalogue.id), summary: `${marketLabel} evidence recorded for ${catalogue.brandName}` }); return { success: true, verifiedAt };
  }),
  createProductSource: protectedProcedure.input(z.object({
    productName: z.string().min(2).max(160), manufacturer: z.string().min(2).max(160), category: z.enum(["neuromodulator", "ha_filler", "biostimulator", "polynucleotide", "lipolysis", "dental_implant", "dental_graft", "dental_membrane", "dental_prosthetic", "dental_other", "other"]), activeIngredient: z.string().max(255).optional(), jurisdiction: z.string().min(2).max(32).default("PL"), language: z.enum(["pl", "en"]).default("pl"), registryAuthority: z.string().max(160).optional(), registryIdentifier: z.string().max(160).optional(), documentTitle: z.string().min(3).max(255), documentUrl: z.string().url().refine(url => url.startsWith("https://"), "Canonical document URL must use HTTPS"), documentVersion: z.string().min(2).max(100), documentKind: z.enum(["spc", "ifu", "pi", "dfu"]), disclosures: z.array(sourceDisclosure).min(1),
  })).mutation(async ({ ctx, input }) => {
