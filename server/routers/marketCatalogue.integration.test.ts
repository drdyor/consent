...
describe("EU product market catalogue", () => {
  it("returns governed discovery records filtered by category and research status", async () => {
    const records = [{ id: 5, brandName: "TEOSYAL range", category: "ha_filler", researchStatus: "curation_ready" }];
    const query: any = { from: () => query, where: (input: unknown) => { state.whereInput = input; return query; }, orderBy: async () => records };
    state.db = { select: vi.fn(() => query) };

    await expect(appRouter.createCaller(ctx as any).marketCatalogue.list({ category: "ha_filler", researchStatus: "curation_ready" })).resolves.toEqual(records);
    expect(state.whereInput).toBeTruthy();
  });
