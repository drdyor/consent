...
export const consentRouter = router({
  ...
  addMapEntry: protectedProcedure.input(z.object({ recordId: z.number().int().positive(), productId: z.number().int().positive(), faceView: z.enum(["front", "left", "right"]), areaKey: z.string().min(2).max(64), coordinateX: z.number().min(0).max(1), coordinateY: z.number().min(0).max(1), measureType: z.enum(["units", "ml", "other"]), amount: z.number().positive().max(9999), clinicalNote: z.string().max(2000).optional() })).mutation(async ({ ctx, input }) => {
    ...
    if (!record[0].productId) throw new Error("This is a procedure-only consent: the product treatment map does not apply");
  ...
  patientHistory: protectedProcedure.input(z.object({ patientId: z.number().int().positive() })).query(async ({ ctx, input }) => {
    ...
    const consents = await db.select({ record: consentRecords, product: products, source: productSources }).from(consentRecords).leftJoin(products, eq(consentRecords.productId, products.id)).leftJoin(productSources, eq(consentRecords.sourceId, productSources.id)).where(and(eq(consentRecords.clinicId, workspace.clinic.id), eq(consentRecords.patientId, patient.id))).orderBy(desc(consentRecords.createdAt));
  ...
  create: protectedProcedure.input(consentInput).mutation(async ({ ctx, input }) => {
    ...
    if (template[0].status !== "active") throw new Error("The selected template is a draft. An administrator must review and activate it before use");
    ...
    const requiresProduct = template[0].requiresProduct;
    let product: { product: typeof products.$inferSelect; source: typeof productSources.$inferSelect } | null = null;
    let inventoryLot: typeof productInventoryLots.$inferSelect | null = null;
    if (requiresProduct) {
      if (!input.productId) throw new Error("This template documents a product-linked procedure: select the product used");
      if (!input.lotNumber || !input.expiryDate) throw new Error("This template documents a product-linked procedure: record the product lot number and expiry date");
      ...
      inventoryLot = input.inventoryLotId ? (await db.select().from(productInventoryLots).where(and(eq(productInventoryLots.id, input.inventoryLotId), eq(productInventoryLots.clinicId, workspace.clinic.id), eq(productInventoryLots.productId, input.productId))).limit(1))[0] || null : null;
      if (input.inventoryLotId && !inventoryLot) throw new Error("The selected inventory lot is not available for this clinic product");
    } else if (input.productId || input.inventoryLotId || input.lotNumber || input.expiryDate) {
      throw new Error("This template declares a procedure-only consent: no product, lot, or expiry may be attached");
    }
    ...
    const created = await db.insert(consentRecords).values({ ...recordInput, patientId, productId: requiresProduct ? input.productId : null, inventoryLotId: requiresProduct ? input.inventoryLotId || null : null, lotNumber: requiresProduct ? inventoryLot?.lotNumber || input.lotNumber : null, expiryDate: requiresProduct ? inventoryLot?.expiryDate || input.expiryDate : null, patientEmail: input.patientEmail || null, clinicId: workspace.clinic.id, templateRevision: template[0].revision, practitionerUserId: ctx.user.id, sourceId: product ? product.source.id : null, status: "draft" }).$returningId();
    const id = created[0]?.id;
    if (!id) throw new Error("Unable to create consent");
    await db.insert(auditEvents).values({ clinicId: workspace.clinic.id, consentRecordId: id, actorUserId: ctx.user.id, action: "consent.created", entityType: "consentRecord", entityId: String(id), summary: requiresProduct ? "Consent draft created" : "Procedure-only consent draft created (template declares no product or device is used)" });
    return { id };
  }),
  ...
  sign: protectedProcedure.input(z.object({ recordId: z.number().int().positive(), signerName: z.string().min(2).max(255), signingMethod: z.enum(["typed", "drawn"]), signatureImageData: z.string().max(7_000_000).optional(), acknowledgedDisclosureIds: z.array(z.number().int().positive()) })).mutation(async ({ ctx, input }) => {
    ...
    const disclosureRows = row.product && row.source ? await db.select().from(disclosureBlocks).where(and(eq(disclosureBlocks.productId, row.product.id), eq(disclosureBlocks.sourceId, row.source.id), eq(disclosureBlocks.language, row.record.language))) : [];
    ...
    const mapEntries = row.product ? await db.select().from(treatmentMapEntries).where(eq(treatmentMapEntries.consentRecordId, row.record.id)).orderBy(treatmentMapEntries.createdAt) : [];
    const signedTreatmentMap = row.product && row.record.lotNumber && row.record.expiryDate ? bindTreatmentMapForSigning(mapEntries, buildTreatmentMapConsentContext({ lotNumber: row.record.lotNumber, expiryDate: row.record.expiryDate }, row.product, row.practitioner)) : [];
    const { snapshot, snapshotHash } = buildSignedSnapshot({ record: row.record, template: { name: row.template.name, revision: row.template.revision, sections: row.template.sections }, product: row.product ?? procedureOnlySnapshotSection(), source: row.source ?? procedureOnlySnapshotSection(), inventoryLot: row.inventoryLot, practitioner: row.practitioner, clinic: workspace.clinic, patient: row.patient, disclosures: applicable, signerName: input.signerName, signingMethod: input.signingMethod, signatureUrl, signedAt, treatmentMap: signedTreatmentMap });
    await db.transaction(async tx => {
      if (required.length) await tx.insert(consentAcknowledgements).values(required.map(d => ({ consentRecordId: row.record.id, disclosureBlockId: d.id, sectionKey: `disclosure-${d.id}`, sectionTitle: d.title, acknowledgedAt: signedAt })));
...
async function getActivePatientSigningLink(token: string) {
  ...
  if (!detail) throw new Error("This patient signing link no longer authorizes an unsigned consent"); const disclosures = detail.product && detail.source ? await db.select().from(disclosureBlocks).where(and(eq(disclosureBlocks.productId, detail.product.id), eq(disclosureBlocks.sourceId, detail.source.id), eq(disclosureBlocks.language, detail.record.language))) : [];
