...
export const consentRouter = router({
  ...
  get: protectedProcedure.input(z.object({ recordId: z.number().int().positive() })).query(async ({ ctx, input }) => {
    ...
    return { ...row, disclosures: disclosureGroups.flat().filter(d => d.scope === "product" || d.treatmentAreaKey === row.record.treatmentAreaKey), mapEntries, photos, courseEntries, materialSelections, educationResourceAttachments };
  }),
