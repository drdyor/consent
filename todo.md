...
- [ ] Implement only the safe Aegis-side additions from the current clinical-platform proposal, retaining governed consent/resource/provenance/operational responsibilities and excluding clinical decision support, diagnostic claims, and patient-state duplication.
- [ ] Write a current Clinic handoff that separates the four proposed clinical pathways, BP/NO observations, research data collection, and referral/patient-communication responsibilities from Aegis’s consent, resource, inventory, and audit responsibilities.
- [ ] Create and verify a reproducible, portable Aegis recovery package containing source history, migrations, database recovery guidance, configuration inventory, test instructions, checksums, and a plain-language statement of remaining non-portable platform dependencies.
- [ ] Synchronize the current Aegis source and handoff documentation to GitHub without overwriting parallel Clinic or Consent work, and record the exact recoverable commit identifiers.
- [ ] Add an explicit clinician-selected, approved link-only education-resource attachment to draft consents, rejecting unapproved, retired, wrong-jurisdiction, or wrong-language references and all patient-trigger/clinical-rule inputs.
- [ ] Seal selected approved education-resource metadata and canonical URLs into the immutable signed-consent snapshot and printable export, without copying third-party guidance text.
- [ ] Surface the selected resource references in consent review and records as non-diagnostic supplementary information, distinct from required product and procedure disclosures.
