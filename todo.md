...
- [x] Implement only the safe Aegis-side additions from the current clinical-platform proposal, retaining governed consent/resource/provenance/operational responsibilities and excluding clinical decision support, diagnostic claims, and patient-state duplication.
- [x] Write a current Clinic handoff that separates the four proposed clinical pathways, BP/NO observations, research data collection, and referral/patient-communication responsibilities from Aegis’s consent, resource, inventory, and audit responsibilities.
- [x] Create and verify a reproducible, portable Aegis recovery package containing source history, migrations, database recovery guidance, configuration inventory, test instructions, checksums, and a plain-language statement of remaining non-portable platform dependencies.
- [x] Synchronize the current Aegis source and handoff documentation to GitHub without overwriting parallel Clinic or Consent work, and record the exact recoverable commit identifiers.
