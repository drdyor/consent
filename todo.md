...
- [x] Group-buy operations handover: Publish the multi-clinic dashboard-to-backend and Aegis-team handover for inventory-aware group buying, RFQ, address revisions, invoice requests, receiving, and assisted-automation safeguards.
- [ ] Aegis portability prerequisite: Complete the currently gated de-Manus migration to portable auth, PostgreSQL, S3/MinIO-compatible storage, durable jobs, and staging contract tests before enabling any live clinical or supplier API.
- [ ] Multi-clinic locations: Add tenant-scoped clinic locations, structured revisioned shipping-address presets, billing/delivery contacts, location permissions, default supplier references, and auditable approval-policy references.
- [ ] Case supply readiness: Build an operational, versioned materials-manifest/readiness API that verifies only user-selected governed product/lot references, source/expiry/quantity state, and policy-defined `ready`/`attention_required`/`blocked` statuses; no clinical recommendation or component substitution.
- [ ] Procurement requests: Add idempotent, case-referenced purchase and invoice/pro-forma request records using selected address presets, operator-managed supplier options, approval states, immutable item/price-source snapshots, and `invoice-issued-not-collected`; no payment or supplier-order submission path.
- [ ] Receiving and status events: Add auditable receiving/reconciliation, lot/expiry movement records, and signed/replay-safe outbox events for stock, procurement, consent, withdrawal, and freshness status changes.
- [ ] Group-buying discovery: Define a deadline-based, direct-ship group-buy model with buying groups, clinic memberships, explicit opt-in commitments, inventory-aware invitations, aggregate demand, supplier RFQ/bid comparison, split requests, and no shared warehousing in the first release.
- [ ] Group-buying safeguards: Define product/category eligibility, urgency exclusions, expiry/holding-time checks, no forced participation, supplier/operator approval, pricing provenance, disclosure of any platform financial interest, and no automated purchase/payment/clinical substitution.
- [ ] Training entitlement boundary: Model only non-clinical training eligibility/entitlement references after an authorized commerce event; do not couple clinical recommendations, consent generation, or implant selection to commercial incentives.
