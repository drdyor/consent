...
| **Patient education** | Determines clinical relevance for a patient, selects an approved item, and records a clinician’s authorisation where required. | May eventually host a neutral, versioned resource library and perform delivery/receipt logging **only after** Clinic supplies an approved resource reference and an explicit communications authority. | Generating patient-specific clinical advice, inferring a condition, choosing an education path, sending autonomously, or cross-selling from health context. |
| **Newsletters / marketing** | Must not contribute chart-derived segmentation through the clinical integration. | A separate future communications-operations service may manage subscriptions, delivery, suppression and audit for independently consented non-clinical outreach. | Combining health data with marketing segmentation, one-click clinical-to-marketing conversion, or automatic email/SMS. |
...
| **Referrals** | Clinician initiates, authors/reviews the letter, chooses recipient/pathway, sends/authorises transmission, and tracks clinical outcome. | A future secure-delivery service may package an already authorised document, send only through a configured endpoint, return a delivery receipt, and retain an immutable transport/audit record. | Referral recommendation, recipient selection, letter generation from health data, autonomous transmission, or patient-data retention beyond the minimum transport evidence. |
| **Research study operations** | Retains clinical observations, results, clinical eligibility, research conduct, and scientific/clinical interpretation. | May govern a separately consented study-enrolment record, kit/consumable lot and shipping logistics, protocol version, reimbursement administration, and data-use-permission audit. | Covert clinical-to-research reuse, outcome prediction, clinical result interpretation, research enrolment by default, or a merged clinical/research patient record. |
| **CPD, subscriptions, and billing** | May show a contextual training link only. | Future operational ownership for enrolment/entitlement evidence, CPD administration, clinic subscriptions, and billing state. | Clinical certification claims without accreditation, payment collection before a separately authorised payment programme, or a clinical entitlement decision. |
...
Patient education selection remains in the clinical app. The safest initial design is deterministic presentation of a clinician-approved, jurisdiction-specific resource selected from a reviewed library. Aegis may later deliver the exact approved artefact and record delivery/view/withdrawal evidence, but receives only resource IDs, purpose, clinic/tenant references, and a signed authorisation—not a diagnosis or medical history. Any personalised generative drafting must be a separate clinical-AI programme with a documented intended-purpose assessment, quality/risk controls, clinician review before sending, user transparency, and immutable action logs. It must not be introduced through the consent or shop feature path.

### 3a. Research and regulated consumables

The proposed saliva/nitrite/nitrate concept is **not** an Aegis consent/shop extension. The Clinic product may own clinical observations and a separately governed research registry may control study protocol and participant eligibility. Aegis’s only plausible later role is operational: research-consent version/provenance, kit catalogue/lot/expiry, study-specific shipping/custody logistics, permitted reimbursement administration, and a separated data-use-permission ledger.

No clinical record may silently become research data. Research participation needs a separate consent and purpose boundary. No kit, device, assessment software, clinical-result interpretation, or medical claim becomes shop-eligible merely because Aegis can track stock; the source/regulatory evidence and the intended-purpose programme remain independent gates.
